...
class InstaBot:
    """
    Instagram common v 1.1.0
    like_per_day=1000 - How many likes set common in one day.
    ...
    https://github.com/LevPasha/instabot
    """
    ...
    def login(self):
        ...
        print(f'Base Csrftoken: {r.cookies["csrftoken"]}')

        time.sleep(5 * random.random())
        login = self.s.post(
            self.url_login, data=self.login_post, allow_redirects=True)
        self.s.headers.update({'X-CSRFToken': login.cookies['csrftoken']})
        print(f'Authenticated Csrftoken: {login.cookies["csrftoken"]}')
        print(f'Cookies after authentication: {self.s.cookies}')
